...
> [!NOTE] مقدمه
> متن How to do Great Work یا همان «چطور کار بزرگ انجام دهیم؟» بهترین متنی است که دربارهٔ کار کردن و انتخاب مسیر شغلی خوانده‌ام. نویسندهٔ متن، پاول گراهام، بنیان‌گذار شتاب‌دهندهٔ [Y Combinator](https://www.ycombinator.com/about) و یکی از باتجربه‌ترین و خرمندترین انسان‌های روزگار است. هدف او از نوشتن این متن طولانی، تهیهٔ فهرستی از تکنیک‌های عالی کار کردن -مستقل از حوزهٔ کاری- است. همین باعث می‌شود که هر مخاطبی با آن ارتباط بگیرد و پس از خواندن متن، چند نکته‌ای برای مسیر خود بردارد. 
> 
> متن با همهٔ خوبی‌هایش، یک ایراد دارد. به خاطر تعدد تکنیک‌ها و توصیه‌ها، ساختارش به هم پیوسته و مرتب نیست. مدام از یک نکته به نکتهٔ دیگر می‌پرد. غلظت متن بسیار بالاست. به خاطر همین محتویاتش فرّار است و باید هرچندوقت یکبار در مقام تذکر دوباره نکاتش را مرور کرد. با مرورهای مکرر، قلاب ذهن هر بار به جایی گیر می‌کند که مسألهٔ حال حاضر است.
> ‌ 
> از آن‌جایی که ترجمهٔ فارسی یا خلاصهٔ کاربردی از این متن وجود ندارد، تصمیم گرفتم که یک دور نکات آن را برای خودم جدا کنم و بنویسم که هر بار مجبور به خواندن کل متن نشوم. در کنار نکات متن، تجربیات خودم و نکات دیگر را هم به یادداشت‌ها اضافه کرده‌ام. علاوه بر این، یادداشت‌برداری از این متنْ مثالی از [[چطور یادداشت برداریم؟|یادداشت‌برداری]] است که برای تازه‌کارها کار را در عمل به نمایش می‌کشد.
> 
> متن جستار به انگلیسی است. ترجمه هم نشده است. با گوگل‌ترنسلیت و سایر ابزارها قابل ترجمه ولی خطاپذیر است. (برای دیدن متن ترجمهٔ ماشینی با مدل `Gemini 2.5 Pro` گوگل [[ترجمهٔ فارسی متن How to Do Great Work|اینجا]] کلیک کنید) تلاش می‌کنم به مرور که در متن جلو می‌رویم، حرف‌های اصلی را هم ترجمه کنم که مطلبی از دست نرود. حرف‌هایی را که به نظرم فرعی می‌رسند یا متصل به حرف‌های بعدی خود هستند ترجمه نمی‌کنم.
...
> 6. [[کار بزرگ چندان برنامه‌ریزی نمی‌خواهد. چرا که اصلاً هدف واضحی ندارد]]*
...
> 27. [[آدم‌های بزرگ هستند که کار بزرگ می‌کنند]]
>
>


> [!NOTE] تعریف کار بزرگ
> داخل متن تعریفی از «کار بزرگ» نشده است. یک علتش این است که تقریباً همهٔ ما آن را می‌شناسیم و منظور متن هم همان کارهایی است که در ذهن همه وجود دارد. اما اگر من بخواهم تعریفی از آن بگویم، کار بزرگ را دو دسته می‌کنم:
> - کار از صفر به یک
> - کار از یک به هزار
> ‌ ‌
> کار صفر به یک از جنس اکتشاف و اختراع است. یعنی انجام آن کاری که پیش از این وجود نداشته است. ساخت پروتوتایپ هم در همین دسته قرار می‌گیرد. کار یک به هزار از جنس توسعه و انبوه‌سازی است. یعنی گرفتن یک ایده و تکثیر آن به حد اعلی.

![[best-interview-question-peter-thiel.png]]
...
> [!NOTE]  کار بزرگ چندان قابل برنامه‌ریزی نیست. چرا که هدف واضحی ندارد
...
> کشف ایده جدید خیلی از اوقات به معنی دیدن حقایقی که دقیقا جلوی چشم ماست که دیده نمی‌شوند. چرا این ایده‌ها دیده نمی‌شوند؟ چون مدل فعلی نگاه ما آن‌ها را نادیده می‌گیرد. اگر [[art#هنر چیست؟|به گونه‌ای دیگر ببینیم]]، ایده‌ها و مسائل خیلی راحت خودشان را نشان می‌دهند.
> ‌ ‌‌ ‌‌ 
